...
# Welcome to DMML Lab 7

In week 7, we studied about partitioning cluster analysis and more specifically, $K$-means clustering and $K$-medoids clustering. 
...
`kmeans` returns an object of class "`kmeans`" which has a `print` and a `fitted` method. It is a list with at least the following components (it is important to understand what each component means):
...
- `clusinfo`: A matrix where each row gives numerical information for one cluster, such as cluster size, maximum dissimilarity within the cluster, average dissimilarity. 
  ...
  - `diameter`: Diameter of the cluster, defined as the maximum dissimilarity between any two data points within a single cluster.
  
  - `separation`: Separation of the cluster, defined as the minimum distance between any data point within a cluster and any data point in a different cluster.

- `isolation`: which clusters are isolated clusters

  - A cluster is an L-cluster iff for each observation i the maximal dissimilarity between i and any other observation of the cluster is smaller than the minimal dissimilarity between i and any observation of another cluster.
